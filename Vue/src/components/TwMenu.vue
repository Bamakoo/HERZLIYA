        ...
        >
          <div class="p-1">
            ...
            >
              <span
                :class="[
                  active ? 'text-gray-700 bg-gray-500' : 'text-gray-500',
                  'flex items-center p-2'
                ]"
                ><component v-if="item.icon" :is="item.icon" class="h-7 w-7 mr-2" /><span>{{
                  item.title
                }}</span></span
              >
            </MenuItem>
...
type RouterItem = { to: string; title: string; icon?: FunctionalComponent }
type ClickItem = {
  onClick: (options: { items: ClickItem; index: number; icon?: FunctionalComponent }) => void
  title: string
}
const props = defineProps<{ items: Array<RouterItem | ClickItem>; btnClass?: string }>()
const actualMenu = window.location.pathname
          ...
          <div class="px-1 py-1">
            <MenuItem v-for="(item, index) in items" :key="index" v-slot="{ active }">
              <component
                ...
                "
                :class="[
                  active || item.to === actualMenu ? 'bg-violet-500 text-white' : 'text-gray-900',
